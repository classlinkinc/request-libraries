...
class OneClick
    ...
    # @param [string] bearer    The bearer token returned by get_token(code)
    #
    # @return [string]          Returns a JSON containing the sourcedId, status, date created or last modified, metdata, a list of orgs, role, username, a list of userIds, first name, last name, middle name, identifier, email, phone and cell numbers, agents, grades, and password.
    # @see https://developer.classlink.com/reference$v2onerostermyinfo
    #
    def get_user_one_roster_info(bearer) 
    ...
    #
    def get_user_one_roster_classes(bearer) 
    ...
    #
    # @param [string] bearer    The bearer token returned by get_token(code)
    # @param [string] sourcedId A string id representing the class gotten from getUserOneRosterClasses(bearer)
    #
    # @return [string]          Returns a JSON containing info listed below
    # @see https://developer.classlink.com/reference$v2onerostermyclassesclass_idteachers
    #
    def get_user_one_roster_class_teachers(bearer, class_sourced_id) 
        return get_info(bearer, "v2/oneroster/my/classes/" + class_sourced_id + "/teachers", "teachers")
    end
    ...
    #
    def get_user_one_roster_class_students(bearer, class_sourced_id) 
        return get_info(bearer, "v2/oneroster/my/classes/" + class_sourced_id + "/students", "students")
    end
end
