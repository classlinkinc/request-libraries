...
module.exports={
  ...
  /**
   * Get all the oneRoster info about the user. Note: oneroster funcitons are
   * only available for districts with oneroster enabled
   * @param bearer The bearer token returned by {@link #getToken(code,cb)}.
   * @param cb The callback function.  Uses error-first callback notation,
   *           will retrun either an error or a JSON containing the user info.
   * @return Returns a JSON containing the sourceId, status, date created or
  ...
  getUserOneRosterInfo:(bearer,cb)=>{
    module.exports.getInfo(bearer,"v2/oneroster/my/info",cb);
  },

  /**
   * Get all the oneRoster classesthe user is enrolled in.  Note: oneroster
   * funcitons are only available for districts with oneroster enabled
   * @param bearer The bearer token returned by {@link #getToken(code,cb)}.
   * @param cb The callback function.  Uses error-first callback notation,
   *           will retrun either an error or JSON containing the classes.
   * @return Returns a JSON containing info listed at @see https://developer.classlink.com/reference#v2onerostermyclasses
   * @since release
   */
  getUserOneRosterClasses:(bearer,cb)=>{
    module.exports.getInfo(bearer,"v2/oneroster/my/classes",cb);
  },

  /**
   * Get info on the teacher of the class represented by a unique
   * classSourcedId.  Note: oneroster funcitons are only available for districts
   * with oneroster enabled
   * @param bearer The bearer token returned by {@link #getToken(code,cb)}.
   * @param classSourcedId a string Id representing the class gotten from
   * {@link getUserOneRosterClasses(bearer,cb)}
   * @param cb The callback function.  Uses error-first callback notation,
   *           will retrun either an error or JSON containing the teacher info.
  ...
  getUserOneRosterClassTeachers:(bearer,classSourcedId,cb)=>{
    module.exports.getInfo(bearer,"v2/oneroster/my/classes/"+classSourcedId+"/teachers","teachers",cb);
  },

  /**
   * Get info on the students enrolled in a class.  Note: oneroster funcitons are
   * only available for districts with oneroster enabled
   * @param bearer The bearer token returned by {@link #getToken(code,cb)}.
   * @param classSourcedId a string Id representing the class gotten from
   * {@link getUserOneRosterClasses(bearer,cb)}
   * @param cb The callback function.  Uses error-first callback notation,
   *           will retrun either an error or list of JSONs containing the
   *           students' info.
  ...
  getUserOneRosterClassStudents:(bearer,classSourcedId,cb)=>{
    module.exports.getInfo(bearer,"v2/oneroster/my/classes/"+classSourcedId+"/students","students",cb);
  },
}
